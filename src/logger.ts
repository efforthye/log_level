...
// logFormat: timestamp [label] level: message
const logFormat = printf(({ level, message, label, timestamp }) => {
    return `${timestamp} [${label}] ${level}: ${message}`; 
});

const logger = winston.createLogger({
   // 로그 출력 형식 정의 (json, label, timestamp, printf, simple, combine)
   format: combine(
   ...
   //  로그, error: 0 , warn: 1 , info: 2 , http: 3 , verbose: 4 , debug: 5 , silly: 6 
   transports: [
        ...
        new winstonDaily({
            ...
            maxFiles: 180, // 최근 180일치 로그 파일을 남김
